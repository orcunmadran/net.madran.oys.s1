...
$deger_sohbetbugun = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_sohbetbugun = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_sohbetbugun = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DS.sohbetgun, DS.sohbetsaat FROM dersler D, dersler_sohbet DS, siniflar S, subeler SU WHERE DS.baslamatarih <= now() AND DS.bitistarih >= now()   AND  DS.subeno = S.subeno  And S.subeno = SU.subeno  AND D.dersno = SU.dersno  AND S.kadi = '%s' AND DS.sohbetgun = DATE_FORMAT(CURDATE(), '%%w')", $deger_sohbetbugun);
$sohbetbugun = mysql_query($query_sohbetbugun, $iysconn) or die(mysql_error());
$row_sohbetbugun = mysql_fetch_assoc($sohbetbugun);
$totalRows_sohbetbugun = mysql_num_rows($sohbetbugun);

$deger_sohbetbugunoe = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_sohbetbugunoe = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_sohbetbugunoe = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DS.sohbetgun, DS.sohbetsaat FROM dersler D, dersler_sohbet DS, subeler SU WHERE DS.baslamatarih <= now() AND DS.bitistarih >= now()  AND  (DS.subeno = SU.subeno AND D.dersno = SU.dersno) AND SU.sorumlu = '%s' AND DS.sohbetgun = DATE_FORMAT(CURDATE(), '%%w')", $deger_sohbetbugunoe);
$sohbetbugunoe = mysql_query($query_sohbetbugunoe, $iysconn) or die(mysql_error());
$row_sohbetbugunoe = mysql_fetch_assoc($sohbetbugunoe);
$totalRows_sohbetbugunoe = mysql_num_rows($sohbetbugunoe);

$deger_tumsohbet = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_tumsohbet = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_tumsohbet = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DS.sohbetgun, DS.sohbetsaat FROM dersler D, dersler_sohbet DS, siniflar S, subeler SU WHERE DS.baslamatarih <= now() AND DS.bitistarih >= now() AND D.dersno = SU.dersno AND  DS.subeno = S.subeno AND S.subeno = SU.subeno  AND S.kadi = '%s'", $deger_tumsohbet);
$tumsohbet = mysql_query($query_tumsohbet, $iysconn) or die(mysql_error());
$row_tumsohbet = mysql_fetch_assoc($tumsohbet);
$totalRows_tumsohbet = mysql_num_rows($tumsohbet);

$deger_tumsohbetoe = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_tumsohbetoe = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_tumsohbetoe = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DS.sohbetgun, DS.sohbetsaat FROM dersler D, dersler_sohbet DS, subeler SU WHERE DS.baslamatarih <= now() AND DS.bitistarih >= now()  AND  (DS.subeno = SU.subeno AND D.dersno = SU.dersno) AND SU.sorumlu = '%s'", $deger_tumsohbetoe);
$tumsohbetoe = mysql_query($query_tumsohbetoe, $iysconn) or die(mysql_error());
$row_tumsohbetoe = mysql_fetch_assoc($tumsohbetoe);
$totalRows_tumsohbetoe = mysql_num_rows($tumsohbetoe);
?>
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="18" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="619" height="28" valign="top"><p class="baslik">Sanak Kampüs'deki sohbetlerin:</p>
           <p class="arabaslik">Bugünkü sohbet / sohbetlerin;</p>
           <table width="100%"  border="0" cellspacing="5" cellpadding="0">
             <?php if ($totalRows_sohbetbugun > 0) { // Show if recordset not empty ?>
             <?php do { ?>
             <tr>
               <td><ul>
                   <li><span class="icyazi"><?php echo $row_sohbetbugun['derskodu']; ?> - <?php echo $row_sohbetbugun['subekodu']; ?> <?php echo $row_sohbetbugun['dersadi']; ?>,
                         <?php
					 $sayilar = array("0", "1", "2", "3", "4", "5", "6");
					 $gunler = array("Pazar", "Pazartesi", "Salý", "Çarþamba", "Perþembe", "Cuma", "Cumartesi");
					 $veriler = $row_sohbetbugun['sohbetgun'];
					 echo str_replace($sayilar, $gunler, $veriler);
					 ?>, <?php echo $row_sohbetbugun['sohbetsaat']; ?></span></li>
               </ul></td>
             </tr>
             <?php } while ($row_sohbetbugun = mysql_fetch_assoc($sohbetbugun)); ?>
             <?php } // Show if recordset not empty ?>
             <?php if ($totalRows_sohbetbugunoe > 0) { // Show if recordset not empty ?>
             <?php do { ?>
             <tr>
               <td><ul>
                   <li><span class="icyazi"><?php echo $row_sohbetbugunoe['derskodu']; ?> - <?php echo $row_sohbetbugunoe['subekodu']; ?> <?php echo $row_sohbetbugunoe['dersadi']; ?>,
                         <?php
					 $sayilar = array("0", "1", "2", "3", "4", "5", "6");
					 $gunler = array("Pazar", "Pazartesi", "Salý", "Çarþamba", "Perþembe", "Cuma", "Cumartesi");
					 $veriler = $row_sohbetbugunoe['sohbetgun'];
					 echo str_replace($sayilar, $gunler, $veriler);
					 ?>, <?php echo $row_sohbetbugunoe['sohbetsaat']; ?> </span></li>
               </ul></td>
             </tr>
             <?php } while ($row_sohbetbugunoe = mysql_fetch_assoc($sohbetbugunoe)); ?>
             <?php } // Show if recordset not empty ?>
           </table>           
           <p class="arabaslik">Bu dönemki sohbet saatlerin;</p>
           <table width="100%"  border="0" cellspacing="5" cellpadding="0">
             <?php if ($totalRows_tumsohbet > 0) { // Show if recordset not empty ?>
             <?php do { ?>
             <tr>
               <td><ul>
                   <li><span class="icyazi"><?php echo $row_tumsohbet['derskodu']; ?> - <?php echo $row_tumsohbet['subekodu']; ?> <?php echo $row_tumsohbet['dersadi']; ?>,
                         <?php
					 $sayilar = array("0", "1", "2", "3", "4", "5", "6");
					 $gunler = array("Pazar", "Pazartesi", "Salý", "Çarþamba", "Perþembe", "Cuma", "Cumartesi");
					 $veriler = $row_tumsohbet['sohbetgun'];
					 echo str_replace($sayilar, $gunler, $veriler);
					 ?>, <?php echo $row_tumsohbet['sohbetsaat']; ?></span></li>
               </ul></td>
             </tr>
             <?php } while ($row_tumsohbet = mysql_fetch_assoc($tumsohbet)); ?>
             <?php } // Show if recordset not empty ?>
             <?php if ($totalRows_tumsohbetoe > 0) { // Show if recordset not empty ?>
             <?php do { ?>
             <tr>
               <td><ul>
                   <li><span class="icyazi"><?php echo $row_tumsohbetoe['derskodu']; ?> - <?php echo $row_tumsohbetoe['subekodu']; ?> <?php echo $row_tumsohbetoe['dersadi']; ?>,
                         <?php
					 $sayilar = array("0", "1", "2", "3", "4", "5", "6");
					 $gunler = array("Pazar", "Pazartesi", "Salý", "Çarþamba", "Perþembe", "Cuma", "Cumartesi");
					 $veriler = $row_tumsohbetoe['sohbetgun'];
					 echo str_replace($sayilar, $gunler, $veriler);
					 ?>, <?php echo $row_tumsohbetoe['sohbetsaat']; ?> </span></li>
               </ul></td>
             </tr>
             <?php } while ($row_tumsohbetoe = mysql_fetch_assoc($tumsohbetoe)); ?>
             <?php } // Show if recordset not empty ?>
           </table>           </td>
...
mysql_free_result($sohbetbugun);

mysql_free_result($sohbetbugunoe);

mysql_free_result($tumsohbet);

mysql_free_result($tumsohbetoe);
?>
