...
$deger_sinavlarbugun = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_sinavlarbugun = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_sinavlarbugun = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DST.baslamatarih, '%%d.%%m.%%y') AS bastrh, DATE_FORMAT(DST.bitistarih, '%%d.%%m.%%y') AS bittrh, DST.* FROM siniflar S, dersler_snvtar DST, dersler D, subeler SU WHERE SU.subeno = DST.subeno AND SU.subeno = S.subeno AND D.dersno = SU.dersno  AND S.kadi = '%s'  AND DST.bitistarih = NOW()", $deger_sinavlarbugun);
$sinavlarbugun = mysql_query($query_sinavlarbugun, $iysconn) or die(mysql_error());
$row_sinavlarbugun = mysql_fetch_assoc($sinavlarbugun);
$totalRows_sinavlarbugun = mysql_num_rows($sinavlarbugun);

$deger_sinavlarbugunoe = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_sinavlarbugunoe = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_sinavlarbugunoe = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DST.baslamatarih, '%%d.%%m.%%y') AS bastrh, DATE_FORMAT(DST.bitistarih, '%%d.%%m.%%y') AS bittrh, DST.* FROM dersler_snvtar DST, dersler D, subeler SU WHERE DST.subeno = SU.subeno AND D.dersno = SU.dersno  AND SU.sorumlu = '%s' AND DST.bitistarih = NOW()", $deger_sinavlarbugunoe);
$sinavlarbugunoe = mysql_query($query_sinavlarbugunoe, $iysconn) or die(mysql_error());
$row_sinavlarbugunoe = mysql_fetch_assoc($sinavlarbugunoe);
$totalRows_sinavlarbugunoe = mysql_num_rows($sinavlarbugunoe);

$deger_tumsinavlar = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_tumsinavlar = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_tumsinavlar = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DST.baslamatarih, '%%d.%%m.%%y') AS bastrh, DATE_FORMAT(DST.bitistarih, '%%d.%%m.%%y') AS bittrh, DST.* FROM siniflar S, dersler_snvtar DST, dersler D, subeler SU WHERE S.subeno = DST.subeno AND S.subeno = SU.subeno AND D.dersno = SU.dersno  AND S.kadi = '%s'  AND DST.bitistarih > NOW()", $deger_tumsinavlar);
$tumsinavlar = mysql_query($query_tumsinavlar, $iysconn) or die(mysql_error());
$row_tumsinavlar = mysql_fetch_assoc($tumsinavlar);
$totalRows_tumsinavlar = mysql_num_rows($tumsinavlar);

$deger_tumsinavlaroe = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_tumsinavlaroe = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_tumsinavlaroe = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DST.baslamatarih, '%%d.%%m.%%y') AS bastrh, DATE_FORMAT(DST.bitistarih, '%%d.%%m.%%y') AS bittrh, DST.* FROM dersler_snvtar DST, dersler D, subeler SU WHERE DST.subeno = SU.subeno AND D.dersno = SU.dersno  AND SU.sorumlu = '%s' AND DST.bitistarih > NOW()", $deger_tumsinavlaroe);
$tumsinavlaroe = mysql_query($query_tumsinavlaroe, $iysconn) or die(mysql_error());
$row_tumsinavlaroe = mysql_fetch_assoc($tumsinavlaroe);
$totalRows_tumsinavlaroe = mysql_num_rows($tumsinavlaroe);
?>
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="18" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="619" height="28" valign="top"><p class="baslik">Sanak Kampüs'deki sýnavlarýn:</p>
           <p class="arabaslik">Bugün girmen gereken sýnav / sýnavlarýn;</p>
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
             <?php if ($totalRows_sinavlarbugun > 0) { // Show if recordset not empty ?>
             <?php do { ?>
             <tr>
               <td><ul>
                   <li><span class="altbaslik"><?php echo $row_sinavlarbugun['derskodu']; ?> - <?php echo $row_sinavlarbugun['subekodu']; ?> <?php echo $row_sinavlarbugun['dersadi']; ?></span><br>
                         <span class="icyazi"><?php echo $row_sinavlarbugun['sinavyeri']; ?> <?php echo $row_sinavlarbugun['sinavtipi']; ?>, <?php echo $row_sinavlarbugun['bastrh']; ?> - <?php echo $row_sinavlarbugun['bittrh']; ?> <?php echo $row_sinavlarbugun['sinavsaat']; ?> </span><br>
                         <span class="formaciklama"><?php echo $row_sinavlarbugun['aciklama']; ?> </span> </li>
               </ul></td>
             </tr>
             <?php } while ($row_sinavlarbugun = mysql_fetch_assoc($sinavlarbugun)); ?>
             <?php } // Show if recordset not empty ?>
             <?php if ($totalRows_sinavlarbugunoe > 0) { // Show if recordset not empty ?>
             <?php do { ?>
             <tr>
               <td><ul>
                   <li><span class="altbaslik"><?php echo $row_sinavlarbugunoe['derskodu']; ?> - <?php echo $row_sinavlarbugunoe['subekodu']; ?> <?php echo $row_sinavlarbugunoe['dersadi']; ?></span><br>
                         <span class="icyazi"><?php echo $row_sinavlarbugunoe['sinavyeri']; ?> <?php echo $row_sinavlarbugunoe['sinavtipi']; ?>, <?php echo $row_sinavlarbugunoe['bastrh']; ?> - <?php echo $row_sinavlarbugunoe['bittrh']; ?> <?php echo $row_sinavlarbugunoe['sinavsaat']; ?> </span><br>
                         <span class="formaciklama"><?php echo $row_sinavlarbugunoe['aciklama']; ?> </span></li>
               </ul></td>
             </tr>
             <?php } while ($row_sinavlarbugunoe = mysql_fetch_assoc($sinavlarbugunoe)); ?>
             <?php } // Show if recordset not empty ?>
           </table>           
           <p class="arabaslik">Ýleri tarihli   sýnav / sýnavlarýn;</p>
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
             <?php do { ?>
             <?php if ($totalRows_tumsinavlar > 0) { // Show if recordset not empty ?>
             <tr>
               <td><ul>
                   <li><span class="altbaslik"><?php echo $row_tumsinavlar['derskodu']; ?> - <?php echo $row_tumsinavlar['subekodu']; ?> <?php echo $row_tumsinavlar['dersadi']; ?></span><br>
                         <span class="icyazi"><?php echo $row_tumsinavlar['sinavyeri']; ?> <?php echo $row_tumsinavlar['sinavtipi']; ?>, <?php echo $row_tumsinavlar['bastrh']; ?> - <?php echo $row_tumsinavlar['bittrh']; ?> <?php echo $row_tumsinavlar['sinavsaat']; ?> </span><br>
                         <span class="formaciklama"><?php echo $row_tumsinavlar['aciklama']; ?> </span></li>
               </ul></td>
             </tr>
             <?php } // Show if recordset not empty ?>
             <?php } while ($row_tumsinavlar = mysql_fetch_assoc($tumsinavlar)); ?>
             <?php do { ?>
             <?php if ($totalRows_tumsinavlaroe > 0) { // Show if recordset not empty ?>
             <tr>
               <td><ul>
                   <li><span class="altbaslik"><?php echo $row_tumsinavlaroe['derskodu']; ?> - <?php echo $row_tumsinavlaroe['subekodu']; ?> <?php echo $row_tumsinavlaroe['dersadi']; ?></span><br>
                         <span class="icyazi"><?php echo $row_tumsinavlaroe['sinavyeri']; ?> <?php echo $row_tumsinavlaroe['sinavtipi']; ?>, <?php echo $row_tumsinavlaroe['bastrh']; ?> - <?php echo $row_tumsinavlaroe['bittrh']; ?> <?php echo $row_tumsinavlaroe['sinavsaat']; ?> </span><br>
                         <span class="formaciklama"><?php echo $row_tumsinavlaroe['aciklama']; ?> </span></li>
               </ul></td>
             </tr>
             <?php } // Show if recordset not empty ?>
             <?php } while ($row_tumsinavlaroe = mysql_fetch_assoc($tumsinavlaroe)); ?>
           </table>           <p class="arabaslik">&nbsp;</p></td>
...
mysql_free_result($sinavlarbugun);

mysql_free_result($sinavlarbugunoe);

mysql_free_result($tumsinavlar);

mysql_free_result($tumsinavlaroe);
?>
