...
mysql_select_db($database_iysconn, $iysconn);
$query_odevlerbugun = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DOP.ttarihi, '%%d.%%m.%%y') AS teslimt, DOP.* FROM dersler D, subeler SU, siniflar S, dersler_op DOP WHERE S.subeno = DOP.subeno AND SU.subeno = S.subeno AND D.dersno = SU.dersno  AND S.kadi = '%s' AND DOP.ttarihi = NOW() AND optipi = 'proje'", $deger_odevlerbugun);
...
mysql_select_db($database_iysconn, $iysconn);
$query_odevlerbugunoe = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DOP.ttarihi, '%%d.%%m.%%y') AS teslimt, DOP.* FROM dersler D, subeler SU, dersler_op DOP WHERE DOP.subeno = SU.subeno AND D.dersno = SU.dersno  AND SU.sorumlu = '%s' AND DOP.ttarihi = NOW() AND optipi = 'proje'", $deger_odevlerbugunoe);
...
mysql_select_db($database_iysconn, $iysconn);
$query_tumodevler = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DOP.ttarihi, '%%d.%%m.%%y') AS teslimt, DOP.* FROM dersler D, subeler SU, siniflar S, dersler_op DOP WHERE S.subeno = DOP.subeno AND SU.subeno = S.subeno AND D.dersno = SU.dersno  AND S.kadi = '%s' AND DOP.ttarihi > NOW() AND optipi = 'proje'", $deger_tumodevler);
...
mysql_select_db($database_iysconn, $iysconn);
$query_tumodevleroe = sprintf("SELECT D.derskodu, SU.subekodu, D.dersadi, DATE_FORMAT(DOP.ttarihi, '%%d.%%m.%%y') AS teslimt, DOP.* FROM dersler D, subeler SU, dersler_op DOP WHERE DOP.subeno = SU.subeno AND D.dersno = SU.dersno  AND SU.sorumlu = '%s' AND DOP.ttarihi > NOW() AND optipi = 'proje'", $deger_tumodevleroe);
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="18" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="619" height="28" valign="top"><p class="baslik">Sanak Kampüs'deki ödevlerin:</p>
           <p class="arabaslik">Teslim edilmesi gereken proje / projeler;</p>
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
             <?php do { ?>
           ...
           <p class="arabaslik">Ýleri tarihli   proje / projeler;</p>
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
             <?php do { ?>
