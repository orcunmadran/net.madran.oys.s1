...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "kaydet")) {
  $updateSQL = sprintf("UPDATE akildefterim SET kadi=%s, `not`=%s, giristarih=%s, hatirlat=%s WHERE adno=%s",
                       GetSQLValueString($_POST['kadi'], "text"),
                       GetSQLValueString($_POST['not'], "text"),
                       GetSQLValueString($_POST['giristarih'], "text"),
                       GetSQLValueString($_POST['hatirlat'], "date"),
...
$degeriki_notal = "-1";
if (isset($_GET['adno'])) {
  $degeriki_notal = (get_magic_quotes_gpc()) ? $_GET['adno'] : addslashes($_GET['adno']);
}
$deger_notal = "-1";
if (isset($_SESSION['MM_Username'])) {
...
mysql_select_db($database_iysconn, $iysconn);
$query_notal = sprintf("SELECT *,  DATE_FORMAT(hatirlat, '%%d.%%m.%%y') AS ht FROM akildefterim WHERE kadi = '%s' AND adno = '%s' ORDER BY adno DESC", $deger_notal,$degeriki_notal);
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="18" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="619" height="28" valign="top"><form action="<?php echo $editFormAction; ?>" method="POST" name="kaydet" id="kaydet">
           <table width="100%"  border="0" cellspacing="5" cellpadding="6">
             <tr>
               <td colspan="3" class="baslik"><a href="yapacaklarim.php">Akýl Defterim</a> &gt; Not Güncelle<span class="baslikuyari"> </span></td>
             </tr>
             <tr class="icyazi">
               <td width="20%"><div align="right"><strong>Not Giriþ Tarihi </strong></div></td>
               <td width="4%"><div align="center"><strong>:</strong></div></td>
               <td width="76%"><?php echo $row_notal['giristarih']; ?></td>
             </tr>
             <tr class="icyazi">
               <td valign="top"><div align="right"><strong>Not</strong></div></td>
               <td valign="top"><div align="center"><strong>:</strong></div></td>
               <td><textarea name="not" cols="65" rows="7" class="icyazi" id="not"><?php echo $row_notal['not']; ?></textarea></td>
             </tr>
             <tr class="icyazi">
               <td><div align="right"><strong>Hatýrlat</strong></div></td>
               <td><div align="center"><strong>:</strong></div></td>
               <td><input name="hatirlat" type="text" class="icyazi" id="hatirlat" value="<?php echo $row_notal['hatirlat']; ?>" size="10">
    ...
    //-->
             ...
             <tr>
               <td><div align="right">
                   <input name="adno" type="hidden" id="adno" value="<?php echo $row_notal['adno']; ?>">
                   <input name="giristarih" type="hidden" id="giristarih" value="<?php echo $row_notal['giristarih']; ?>">
                   <input name="kadi" type="hidden" id="kadi" value="<?php echo $row_gk['kadi']; ?>">
               </div></td>
               <td>&nbsp;</td>
               <td><input name="Submit" type="submit" class="butongiris" onClick="MM_callJS('d')" value="Notu Güncelle"></td>
             </tr>
           </table>
             <p>
             </p>
             <input type="hidden" name="MM_update" value="kaydet">
         </form>         
