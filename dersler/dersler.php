...
$deger_ydersler = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_ydersler = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_ydersler = sprintf("SELECT DISTINCT DE.derskodu, DE.dersadi, SU.subekodu, SU.sorumlu, SU.subeno FROM dersler DE, subeler SU, siniflar S WHERE DE.dersno = SU.dersno  AND SU.subeno = S.subeno AND S.kadi = '%s' ORDER BY DE.derskodu", $deger_ydersler);
$ydersler = mysql_query($query_ydersler, $iysconn) or die(mysql_error());
$row_ydersler = mysql_fetch_assoc($ydersler);
$totalRows_ydersler = mysql_num_rows($ydersler);

$deger_ydersleroe = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_ydersleroe = (get_magic_quotes_gpc()) ? $_SESSION['MM_Username'] : addslashes($_SESSION['MM_Username']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_ydersleroe = sprintf("SELECT DISTINCT DE.derskodu, DE.dersadi, SU.subekodu, SU.sorumlu, SU.subeno FROM dersler DE, subeler SU WHERE DE.dersno = SU.dersno  AND SU.sorumlu = '%s'", $deger_ydersleroe);
$ydersleroe = mysql_query($query_ydersleroe, $iysconn) or die(mysql_error());
$row_ydersleroe = mysql_fetch_assoc($ydersleroe);
$totalRows_ydersleroe = mysql_num_rows($ydersleroe);
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="18" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td colspan="2" class="icyazi"><span class="baslik">
           <?php if ($totalRows_ydersler > 0) { // Show if recordset not empty ?>
Bu dönem yüklenmiþ olduðun dersler
<?php } // Show if recordset not empty ?>
           <?php if ($totalRows_ydersleroe > 0) { // Show if recordset not empty ?>
Bu dönem sorumlusu olduðunuz dersler
<?php } // Show if recordset not empty ?>
         </span></td>
        </tr>
       <tr>
         <td width="70%" class="icyazi"><?php if ($totalRows_ydersler > 0) { // Show if recordset not empty ?>
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
             <tr class="icyazi">
               <td width="10%"><div align="center"><strong>Ders Kodu </strong></div></td>
               <td width="33%"><div align="left"><strong>Ders Adý </strong></div></td>
               <td width="10%"><div align="center"><strong>Sube Kodu</strong></div></td>
               <td width="15%"><div align="center"><strong>Ders Sorumlusu</strong></div></td>
             </tr>
             <?php do { ?>
             <tr class="icyazi">
               <td><div align="center"><?php echo $row_ydersler['derskodu']; ?></div></td>
               <td><div align="left"><a href="ders.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."subeno=".urlencode($row_ydersler['subeno']) ?>"><?php echo $row_ydersler['dersadi']; ?></a></div></td>
               <td><div align="center"><?php echo $row_ydersler['subekodu']; ?></div></td>
               <td><div align="center"><?php echo $row_ydersler['sorumlu']; ?></div></td>
             </tr>
             <?php } while ($row_ydersler = mysql_fetch_assoc($ydersler)); ?>
           </table>
           <?php } // Show if recordset not empty ?>
           <?php if ($totalRows_ydersleroe > 0) { // Show if recordset not empty ?>
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
              ...
              <?php do { ?>
              <tr class="icyazi">
                <td><div align="center"><?php echo $row_ydersleroe['derskodu']; ?></div></td>
                <td><div align="left"><a href="ders.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."subeno=".urlencode($row_ydersleroe['subeno']) ?>"><?php echo $row_ydersleroe['dersadi']; ?></a></div></td>
                <td><div align="center"><?php echo $row_ydersleroe['subekodu']; ?></div></td>
                <td><div align="center"><?php echo $row_ydersleroe['sorumlu']; ?></div></td>
              </tr>
              <?php } while ($row_ydersleroe = mysql_fetch_assoc($ydersleroe)); ?>
            </table>
...
mysql_free_result($ydersler);

mysql_free_result($ydersleroe);
?>
