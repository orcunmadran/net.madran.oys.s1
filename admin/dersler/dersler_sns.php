...
if ((isset($_GET['snvtrhno'])) && ($_GET['snvtrhno'] != "")) {
  $deleteSQL = sprintf("DELETE FROM dersler_snvtar WHERE snvtrhno=%s",
                       GetSQLValueString($_GET['snvtrhno'], "int"));

  mysql_select_db($database_iysconn, $iysconn);
  $Result1 = mysql_query($deleteSQL, $iysconn) or die(mysql_error());

  $deleteGoTo = "dersler_snb.php?bilgi= > Sýnav Bilgisi baþarýyla silindi.";
