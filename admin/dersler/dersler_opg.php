...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "yayinla")) {
  $updateSQL = sprintf("UPDATE dersler_op SET subeno=%s, optipi=%s, opadi=%s, aciklama=%s, ttarihi=%s WHERE opno=%s",
                       GetSQLValueString($_POST['subeno'], "int"),
                       GetSQLValueString($_POST['optipi'], "text"),
                       GetSQLValueString($_POST['opadi'], "text"),
                       GetSQLValueString($_POST['aciklama'], "text"),
                       GetSQLValueString($_POST['ttarihi'], "date"),
                       GetSQLValueString($_POST['opno'], "int"));

  mysql_select_db($database_iysconn, $iysconn);
  $Result1 = mysql_query($updateSQL, $iysconn) or die(mysql_error());

  $updateGoTo = "dersler_opg.php?bilgi= > Kayýt baþarýyla güncellendi";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_op = "-1";
if (isset($_GET['opno'])) {
  $colname_op = (get_magic_quotes_gpc()) ? $_GET['opno'] : addslashes($_GET['opno']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_op = sprintf("SELECT *,  DATE_FORMAT(ttarihi, '%%d.%%m.%%y') AS tt FROM dersler_op WHERE opno = %s", $colname_op);
$op = mysql_query($query_op, $iysconn) or die(mysql_error());
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="14" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="77%" class="baslik"><a href="dersler.php"><?php echo $row_ders['desu']; ?></a> Ödev / Proje Bilgisi Güncelle <span class="baslikuyari"><?php echo $_GET['bilgi']; ?></span></td>
         <td width="23%">&nbsp;</td>
       </tr>
       <tr>
         <td colspan="2"><form action="<?php echo $editFormAction; ?>" method="POST" name="yayinla" id="yayinla">
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
             <tr class="icyazi">
               <td width="18%"><div align="right"><strong>Ödev  / Proje</strong></div></td>
               <td width="2%"><div align="center"><strong>:</strong></div></td>
               <td width="80%"><select name="optipi" class="icyazi" id="optipi">
                 <option value="<?php echo $row_op['optipi']; ?>">Deðiþiklik yok</option>
                 <option value="odev">Ödev</option>
                 <option value="proje">Proje</option>
                                                            </select>
                  <?php echo $row_op['optipi']; ?></td>
             </tr>
             <tr class="icyazi">
               <td><div align="right"><strong>Ödev / Proje Adý </strong></div></td>
               <td><div align="center"><strong>:</strong></div></td>
               <td><input name="opadi" type="text" class="icyazi" id="opadi" value="<?php echo $row_op['opadi']; ?>" size="50" maxlength="255"></td>
             </tr>
             <tr class="icyazi">
               <td valign="top"><div align="right"><strong>Açýklama</strong></div></td>
               <td valign="top"><div align="center"><strong>:</strong></div></td>
               <td><textarea name="aciklama" cols="50" rows="5" class="icyazi" id="aciklama"><?php echo $row_op['aciklama']; ?></textarea></td>
             </tr>
             <tr class="icyazi">
               <td valign="top"><div align="right"><strong>Teslim Tarihi </strong></div></td>
               <td valign="top"><strong>:</strong></td>
               <td><input name="ttarihi" type="text" class="icyazi" id="ttarihi" value="<?php echo $row_op['ttarihi']; ?>" size="15">
                 <strong>
                 <script language="JavaScript" type="text/javascript">
        		function calendar1Callback(date, month, year)
        			{
            		document.yayinla.ttarihi.value = year + '-' + month + '-' + date;
        			}
        			calendar1 = new dynCalendar('calendar1', 'calendar1Callback');
    			 
                 </script>
&nbsp; <?php echo $row_op['tt']; ?></strong></td>
             </tr>
             <tr class="icyazi">
               <td valign="top"><div align="right">
                 <input name="opno" type="hidden" id="opno" value="<?php echo $row_op['opno']; ?>">
                 <input name="subeno" type="hidden" id="subeno" value="<?php echo $row_op['subeno']; ?>">
               </div></td>
               <td valign="top">&nbsp;</td>
               <td><input name="Submit" type="submit" class="butongiris" onClick="YY_checkform('yayinla','opadi','#q','0','Ödev / Proje adi boþ býrakýlamaz.');return document.MM_returnValue" value="Ödev / Proje Bilgisi Güncelle"></td>
             </tr>
           </table>
       ...
       <tr>
         <td colspan="2"><?php if ($totalRows_op > 0) { // Show if recordset not empty ?>
           <table width="100%"  border="0" cellspacing="5" cellpadding="5">
             <tr class="icyazi">
                <td width="13%"><div align="center"><strong>Ödev / Proje</strong></div></td>
                <td width="57%"><div align="left"><strong>Ödev / Proje Adý </strong></div>
                <div align="center"></div></td>
                <td width="14%"><div align="center"><strong>Teslim Tarihi </strong></div></td>
                <td width="11%"><div align="center"><strong>Güncelle</strong></div></td>
                <td width="5%"><div align="center"><strong>Sil</strong></div></td>
             </tr>
             <?php do { ?>
             <tr class="icyazi">
                <td><div align="center"><?php echo $row_op['optipi']; ?></div></td>
                <td><div align="center"></div>                  <div align="left"><?php echo $row_op['opadi']; ?> </div></td>
                <td><div align="center"><?php echo $row_op['tt']; ?></div></td>
                <td><div align="center"> <a href="dersler_opg.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."opno=".urlencode($row_op['opno']) ?>"><img src="../../resimler/guncelle.gif" width="16" height="16" border="0"></a> </div></td>
                <td><div align="center"> <a href="dersler_ops.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."opno=".urlencode($row_op['opno']) ?>"><img src="../../mesajlar/m_imaj/sil.gif" width="16" height="14" border="0"></a> </div></td>
             </tr>
             <?php } while ($row_op = mysql_fetch_assoc($op)); ?>
           </table>
