...
$colnameiki_mesajlar = "-1";
if (isset($_GET['kadi'])) {
  $colnameiki_mesajlar = (get_magic_quotes_gpc()) ? $_GET['kadi'] : addslashes($_GET['kadi']);
}
$colname_mesajlar = "-1";
if (isset($_GET['kadi'])) {
  $colname_mesajlar = (get_magic_quotes_gpc()) ? $_GET['kadi'] : addslashes($_GET['kadi']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_mesajlar = sprintf("SELECT *, DATE_FORMAT(tarihsaat, '%%d.%%m.%%y - %%H:%%i') AS ts FROM mesajlar WHERE alici = '%s' OR gonderen ='%s' ORDER BY tarihsaat DESC", $colname_mesajlar,$colnameiki_mesajlar);
$mesajlar = mysql_query($query_mesajlar, $iysconn) or die(mysql_error());
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="14" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td colspan="3" class="baslik"> <span class="arabaslikuyari"><?php echo $_GET['kadi'] ?></span> kullanýcýsýnýn aldýðý ve attýðý tüm mesajlar </td>
       </tr>
       <tr>
         <td width="24%" class="icyazi"><strong>Toplam Mesaj Sayýsý: </strong><span class="icyaziuyari"><?php echo $totalRows_mesajlar?></span></td>
         <td width="48%" class="icyazi"><span class="icyaziuyari"><?php echo $_GET['bilgi']?></span>&nbsp;</td>
         <td width="28%" class="icyazi"><div align="right"><a href="javascript:history.back()">Kullanýcý Bilgilerine geri dön</a></div></td>
       </tr>
       <tr>
         <td colspan="3"><form method="POST" name="goster" id="goster">
           <table width="100%"  border="0" cellspacing="5" cellpadding="0">
             <tr>
               <td width="4%">&nbsp;</td>
               <td width="19%" class="arabaslik">Gönderen</td>
               <td width="19%" class="arabaslik">Alýcý</td>
               <td width="40%" class="arabaslik">Konu</td>
               <td width="18%" class="arabaslik">Tarih</td>
             </tr>
             <tr>
               <td>&nbsp;</td>
               <td colspan="4" class="altbaslik"><hr size="1"></td>
               </tr>
             <?php do { ?>
             <tr>
                  <td><?php if ($totalRows_mesajlar > 0) { // Show if recordset not empty ?>
                    <img src="../../mesajlar/m_imaj/m<?php echo $row_mesajlar['okundu']; ?>.gif">
                    <?php } // Show if recordset not empty ?></td>
                  <td class="icyazi"><?php echo $row_mesajlar['gonderen']; ?></td>
                  <td class="icyazi"><?php echo $row_mesajlar['alici']; ?></td>
                  <td class="icyazi"><a href="mesaj.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."mesajno=".urlencode($row_mesajlar['mesajno']) ?>"><?php echo $row_mesajlar['konu']; ?></a></td>
                  <td class="icyazi"><?php echo $row_mesajlar['ts']; ?></td>
             </tr>
