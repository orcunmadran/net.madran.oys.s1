...
if (isset($_GET['girisno'])) {
  $deger_basliklar = (get_magic_quotes_gpc()) ? $_GET['girisno'] : addslashes($_GET['girisno']);
}
mysql_select_db($database_iysconn, $iysconn);
$query_basliklar = sprintf("SELECT F.adi, B.*, DATE_FORMAT(B.tarih, '%%d.%%m.%%y - %%H:%%i') AS trh FROM tg_forumlar F, tg_basliklar B WHERE F.girisno = B.girisno AND F.aktif = 'Evet' AND B.aktif = 'Evet' AND B.girisno = %s", $deger_basliklar);
$basliklar = mysql_query($query_basliklar, $iysconn) or die(mysql_error());
$row_basliklar = mysql_fetch_assoc($basliklar);
$totalRows_basliklar = mysql_num_rows($basliklar);
  ...
  <tr><!-- row 3 -->
   ...
   <td colspan="18" valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="icerik" -->
     <table width="100%"  border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td class="icyazi"><span class="baslik">Forum Baþlýklarý</span></td>
         <td class="icyazi">&nbsp;</td>
       </tr>
       <tr>
         <td width="70%" class="icyazi"><p class="arabaslik"> <a href="forumlar.php">Forumlar</a> &gt; <?php echo $row_basliklar['adi']; ?> </p></td>
         <td width="30%" class="icyazi"><div align="right"></div></td>
       </tr>
       <tr>
         <td colspan="2" class="icyazi"><table width="100%"  border="0" cellpadding="5" cellspacing="2" bgcolor="#8093B3">
           <tr bgcolor="#CCCCCC" class="icyazi">
             <td width="43%" nowrap><strong>Baþlýk</strong></td>
             <td width="28%" nowrap><strong>Yazar</strong></td>
             <td width="16%" nowrap><div align="center"><strong>Oluþturulma Tarihi</strong></div></td>
             <td width="13%" nowrap><div align="center"><strong>Yorum Ekle</strong></div></td>
           </tr>
           <tr bgcolor="#FFFFFF" class="icyazi">
             <td colspan="4" nowrap><div align="left">
               <?php if ($totalRows_basliklar == 0) { // Show if recordset empty ?>
               Bu forum altýnda henüz bir baþlýk oluþturulmamýþtýr.
               <?php } // Show if recordset empty ?>
             </div></td>
             </tr>
           <?php do { ?>
           <tr bgcolor="#FFFFFF" class="icyazi">
               <td><a href="yorumlar.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."baslikno=".urlencode($row_basliklar['baslikno']) ?>"><?php echo $row_basliklar['baslik']; ?></a></td>
               <td><?php echo $row_basliklar['yazar']; ?></td>
               <td><div align="center"><?php echo $row_basliklar['trh']; ?></div></td>
               <td><div align="center"><a href="yorumekle.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."baslikno=".urlencode($row_basliklar['baslikno']) ?>">
                 <?php if ($totalRows_basliklar > 0) { // Show if recordset not empty ?>
                 <img src="../resimler/ekle.gif" width="26" height="26" border="0">
